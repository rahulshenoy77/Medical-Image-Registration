...
class LocalNet(BaseNet):
    ...
    def __init__(self, ddf_levels=None, **kwargs):
        ...
        self.hm = [layer.conv3_block(h3, nc[3], nc[4], name='local_deep_4')]

        min_level = min(self.ddf_levels)
        self.hm += [layer.upsample_resnet_block(self.hm[0], hc3, nc[4], nc[3], name='local_up_3')] if min_level < 4 else []
        self.hm += [layer.upsample_resnet_block(self.hm[1], hc2, nc[3], nc[2], name='local_up_2')] if min_level < 3 else []
        self.hm += [layer.upsample_resnet_block(self.hm[2], hc1, nc[2], nc[1], name='local_up_1')] if min_level < 2 else []
        self.hm += [layer.upsample_resnet_block(self.hm[3], hc0, nc[1], nc[0], name='local_up_0')] if min_level < 1 else []

        self.ddf = tf.reduce_sum(tf.stack([layer.ddf_summand(self.hm[4-idx], nc[idx], self.image_size, name='sum_%d' % idx)
